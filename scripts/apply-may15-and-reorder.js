// Réplication "The Bettor's Daily — May 15, 2026 / Le Journal du Parieur — 15 mai"
// depuis bigwin vers les 4 autres apps + réordonnancement sur les 5 apps :
//   0 → May 15 Journal     1 → May 14 Journal     2 → May 13 Journal
//   3 → Les 3 Secrets      4 → 5 erreurs fatales  5 → 30 Prompts
//   6 → 7 BUSINESS
// (PSG-Arsenal reste à so=99, isActive=false — non touché)
//
// Idempotent : skip si déjà au bon sortOrder, skip création si May 15 déjà présent.
...
const ORDER = [
  { regex: { fr: /15 mai/i, en: /may 15/i },               so: 0, label: '🔥 May 15 Journal' },
  { regex: { fr: /14 mai/i, en: /may 14/i },               so: 1, label: '🔥 May 14 Journal' },
  { regex: { fr: /13 mai/i, en: /may 13/i },               so: 2, label: '🔥 May 13 Journal' },
  { regex: { fr: /3 Secrets/i, en: /3 Secrets/i },         so: 3, label: 'Les 3 Secrets' },
  { regex: { fr: /5 erreurs|erreurs fatales/i, en: /5 fatal|fatal mistakes/i }, so: 4, label: '5 erreurs fatales' },
  { regex: { fr: /30 Prompts/i, en: /30 Prompts/i },       so: 5, label: '30 Prompts' },
  { regex: { fr: /7 BUSINESS/i, en: /7 BUSINESS/i },       so: 6, label: '7 BUSINESS' },
];
...
(async () => {
  ...
  // Trouve le gift du 15 mai sur bigwin (créé manuellement par l'user)
  ...
  if (!src) {
    console.error('❌ May 15 Journal source introuvable sur bigwin');
    process.exit(1);
  }
  ...
  for (const appId of OTHER_APPS) {
    ...
    if (existing) {
      console.log(`  ⏭️  May 15 Journal déjà présent (${existing._id})`);
    } else {
      const clone = { ...src };
      ...
      console.log(`  ✅ Cloné May 15 Journal → ${created._id} (so=0)`);
    }
    await applyOrder(appId);
  }

  // Vérif finale
  console.log('\n=== Vérif finale (top 8 par sortOrder) ===');
  for (const appId of ALL_APPS) {
    const top = await Gift.find({ appId }).sort({ sortOrder: 1, createdAt: -1 }).limit(8).lean();
    console.log(`\n  ${appId} :`);
    for (const g of top) {
