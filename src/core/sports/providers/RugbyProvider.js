/**
 * @fileoverview Fournisseur de données pour le rugby
 */
const SportProvider = require('./SportProvider');

/**
 * Fournisseur de données pour le rugby
 * @extends SportProvider
 */
class RugbyProvider extends SportProvider {
  /**
  ...
  async fetchFixtures(date) {
    try {
      this.logger.info(`Fetching rugby games for ${date}`);
  ...
  normalizeData(rawData) {
    ...
    const matches = games.map(game => {
      ...
      return {
        ...
        venue: null, // Pas de détails de stade dans l'API rugby
        ...
        score: {
          ...
          details: {
            home: {
              ...
              overtime: game.periods.overtime.home,
              second_overtime: game.periods.second_overtime.home
            },
            away: {
              first_half: game.periods.first.away,
              second_half: game.periods.second.away,
              overtime: game.periods.overtime.away,
              second_overtime: game.periods.second_overtime.away
            }
          }
        },
        sportSpecific: {
          ...
          periods: {
            ...
            second_overtime: game.periods.second_overtime
          }
        }
      };
    });
