/**
 * @fileoverview Fournisseur de données pour le hockey
 */
const SportProvider = require('./SportProvider');

/**
 * Fournisseur de données pour le hockey
 * @extends SportProvider
 */
class HockeyProvider extends SportProvider {
  /**
  ...
  async fetchFixtures(date) {
    try {
      this.logger.info(`Fetching hockey games for ${date}`);
  ...
  normalizeData(rawData) {
    ...
    const matches = games.map(game => {
      ...
      return {
        ...
        venue: null, // Pas de détails de stade dans l'API hockey
        ...
        score: {
          ...
          details: {
            home: {
              ...
              third_period: game.periods.third,
